...
exports.handleRequest = function(req,res){
    ...
    try{
        if(handle instanceof Array){
            var h = handle.shift();
            h.apply(this,handle.concat([req,res]));
        }else{
            on404(req,res);
        }
    }catch(ex){
    ...
    var path = paths.pop();
        ...
        try{
            ...
            if(handle[path]){
                ...
                console.log('无法找到 ./'+paths.join('/')+'的'+path+'方法');
            }
            //判断handle是否为默认方法
            if(handle instanceof Function){
                return [handle,path];
            }else{
                console.log('无法找到 ./'+paths.join('/')+'的默认方法方法');
            }
        }catch(ex){
            console.log('无法找到 ./'+paths.join('/')+'模块');
        }
        //第三种情况
        ...
        try{
            ...
            if(handle[method]){
                ...
                console.log('无法找到 ./'+paths.join('/')+'的'+method+'方法');
            }
